    ...
    try:
        ...
        while True:
            ...
            if save_rules.upper() == 'Y':
                if len(udev_rules) > 0:
                    with io.FileIO("58-nour.rules", "w") as file:
                        file.write(udev_rules)
                    print "\r\n58-nour.rules saved."
                    print "\r\nRUN: $ sudo cp 58-nour.rules /etc/udev/rules.d"
                    print "\r\nRestart Restart udev:"
                    print "\r\nRUN: $ sudo service udev reload"
                    print "\r\nRUN: $ sudo service udev restart"
                    print "\r\nConfirm if the uDev rules worked:"
                    print "\r\nRUN: $ ls /dev/nourbase"
                    print "\r\nRUN: $ ls /dev/nourlidar"
                    print "\r\nRestart the computer once done if needed."
                    break
                else:
                    break
